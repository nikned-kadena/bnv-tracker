...
def extract_agencija(card):
    """Izvuci naziv agencije/oglašivača sa kartice."""
    # Pokušaj različite selektore koje Halo Oglasi koristi
    for sel in [
        {"class": re.compile(r"username|advertiser-name|agency-name|oglasivac")},
        {"class": re.compile(r"offer-adress")},
        {"itemprop": "name"},
    ]:
        el = card.find(attrs=sel)
        if el:
            txt = el.get_text(strip=True)
            if txt and len(txt) > 1 and len(txt) < 80:
                return txt

    # Fallback — traži img alt sa imenom agencije
    for img in card.find_all("img", alt=True):
        alt = img.get("alt", "").strip()
        if alt and len(alt) > 2 and len(alt) < 80:
            # Preskoči generičke altove
            if not any(x in alt.lower() for x in ["logo", "slika", "photo", "halo"]):
                return alt
