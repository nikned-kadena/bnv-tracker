#!/usr/bin/env python3
"""
BnV Tracker v4.15 — ispravni dnevni diff, overwrite history po danu
"""
...
def scraper_get(url, render_js=False):
    params = {
...
def parse_page(html, page_num, mode="prodaja"):
    ...
    for card in cards:
        try:
            ...
            if has_bld:
                from buildings import is_blacklisted
                if is_blacklisted(title, full_context, street_text):
                    continue
...
def scrape_mode(mode):
    ...
    empty_streak = 0  # Broj uzastopnih praznih stranica
...
def diff_listings(prev, curr):
    """Diff između dva lista — novi su oni čiji dedup_key nije bio u prev."""
...
def load_eod_snapshot(mode, date_str):
    """Učitaj snapshot od kraja prethodnog dana (eod = end of day)."""
    path = DATA_DIR / f"eod_{mode}.json"
    if path.exists():
        data = json.load(open(path, encoding="utf-8"))
        # Koristi eod samo ako je od prethodnog dana
        if data.get("date") != date_str:
            return data
    # Fallback na latest ako eod ne postoji
    latest_path = DATA_DIR / f"latest_{mode}.json"
    if latest_path.exists():
        data = json.load(open(latest_path, encoding="utf-8"))
        # Ako je latest od danas, pokušaj da nađemo juče iz snapshotsova
        if data.get("scraped_at","")[:10] == date_str:
            # Traži snapshot od juče
...
def save_snapshot(mode, all_listings, total_raw):
    ...
    # eod snapshot — čuva se samo ako još nema eod za danas
    # (prvi run dana postaje referenca za diff tokom dana)
    eod_path = DATA_DIR / f"eod_{mode}.json"
    if not eod_path.exists() or json.load(open(eod_path,encoding="utf-8")).get("date") != date_str:
        # Novi dan — spasi prethodni latest kao eod referensu
        if eod_path.exists():
            pass  # već postoji od prethodnog dana — ne diramo
        else:
            # Inicijalni run — snapshot je i eod
            with open(eod_path,"w",encoding="utf-8") as f:
                json.dump(snapshot,f,ensure_ascii=False,indent=2)
    # Uvek ažuriraj eod na kraju dana (poslednji run je referenca za sutra)
    with open(eod_path,"w",encoding="utf-8") as f:
        eod_data = {**snapshot, "date": date_str}
        json.dump(eod_data,f,ensure_ascii=False,indent=2)

    # History — overwrite po danu
...
def main():
    print("="*55)
    print(f"BnV Scraper v4.15 — {datetime.now().strftime('%Y-%m-%d %H:%M UTC')}")
