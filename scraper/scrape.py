...
def extract_agencija(card):
    ...
    # Primarni selektor: div.product-logo > img alt atribut
    logo_div = card.find("div", class_=re.compile(r"product-logo"))
    if logo_div:
        img = logo_div.find("img", alt=True)
        if img:
            alt = img.get("alt", "").strip()
            if alt and 2 < len(alt) < 80:
                return alt
        # Fallback: tekst u logo divu
        txt = logo_div.get_text(strip=True)
