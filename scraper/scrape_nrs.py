...
def fetch(url: str, session: requests.Session, render: bool = False) -> BeautifulSoup | None:
    ...
    for attempt in range(1, 4):
        try:
            r = session.get(url, headers=DIRECT_HEADERS, timeout=30)
            print(f"    Fetch {url[:80]} → {r.status_code}")
...
def parse_listing_page(soup: BeautifulSoup) -> list[dict]:
    ...
    # DEBUG — ispisi strukturu
    all_tags = {}
    for tag in soup.find_all(True):
        name = tag.name
        all_tags[name] = all_tags.get(name, 0) + 1
    top = sorted(all_tags.items(), key=lambda x: -x[1])[:15]
    print(f"    DEBUG HTML tagovi: {top}")
    links_oglasi = soup.select("a[href*='/oglasi/']")
    print(f"    DEBUG /oglasi/ linkovi: {len(links_oglasi)}")
    if links_oglasi:
        print(f"    DEBUG prvi link: {links_oglasi[0].get('href')}")
    articles_all = soup.select("article")
    print(f"    DEBUG article tagovi: {len(articles_all)}")
    # Pogledaj prvih 2000 karaktera HTML-a
    print(f"    DEBUG HTML preview: {str(soup)[:500]}")
