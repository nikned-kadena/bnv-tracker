...
# Stranica 1: bez parametra, stranice 2+: ?strana=N
BASE_PRODAJA    = "https://www.nekretnine.rs/prodaja-stanova/beograd/beograd-na-vodi-palata-pravde/?strana={}"
BASE_RENTA      = "https://www.nekretnine.rs/izdavanje-stanova/beograd/beograd-na-vodi-palata-pravde/?strana={}"
BASE_PRODAJA_P1 = "https://www.nekretnine.rs/prodaja-stanova/beograd/beograd-na-vodi-palata-pravde/"
BASE_RENTA_P1   = "https://www.nekretnine.rs/izdavanje-stanova/beograd/beograd-na-vodi-palata-pravde/"
...
def fetch(url: str, session: requests.Session, render: bool = False, page: int = 1) -> BeautifulSoup | None:
    """Fetch URL kroz ScraperAPI sa render=true.
    Za stranice > 1 koristi instruction set da klikne na link stranice u paginaciji.
    """
    # Base params
    params = {
        "api_key": API_KEY,
        "url":     url,
        "render":  "true",
        "wait_for_selector": "a[href*='/oglasi/']",
    }

    # Za stranice > 1: klikni na broj stranice u paginatoru
    if page > 1:
        instruction = _json.dumps([
            {"type": "wait_for_selector",
             "selector": {"type": "css", "value": f"a[aria-label='{page}'], a[data-page='{page}'], .pagination a:nth-child({page})"}},
            {"type": "click",
             "selector": {"type": "css", "value": f"a[aria-label='{page}'], a[data-page='{page}'], .pagination a:nth-child({page})"}},
            {"type": "wait_for_selector",
             "selector": {"type": "css", "value": "a[href*='/oglasi/']"}},
        ])
        params["instruction_set"] = instruction

    for attempt in range(1, 4):
        try:
            r = session.get(SCRAPER_URL, params=params, timeout=90)
            print(f"    Fetch {url} (p{page}) → {r.status_code}")
            if r.status_code == 200 and len(r.text) > 500:
                return BeautifulSoup(r.text, "html.parser")
            if r.status_code == 429:
                print(f"    Rate limit — čekam {15 * attempt}s")
                time.sleep(15 * attempt)
            else:
                print(f"    Status {r.status_code} — pokušaj {attempt}/3")
                time.sleep(5 * attempt)
        except Exception as e:
            print(f"    Greška: {e} (pokušaj {attempt})")
            time.sleep(5 * attempt)
    return None
...
def parse_listing_page(soup: BeautifulSoup) -> list[dict]:
    ...
    # nekretnine.rs koristi Next.js — nema <article>, oglasi su u <a href="/oglasi/ID/">
    # Izvuci sve unique oglas linkove sa stranice
    seen = set()
    for a in soup.select("a[href]"):
        href = a.get("href", "")
        # Apsolutni ili relativni URL ka /oglasi/ID/
        ...
        # Pokusaj da izvucemo osnovne podatke iz kontejnera linka
        parent = a
        for _ in range(5):  # idi gore max 5 nivoa
            if parent.parent:
                parent = parent.parent
            if len(parent.select("a[href*='/oglasi/']")) >= 1:
                break
        ...
        for sel in ["h2", "h3", "h4", "[class*='title']", "[class*='name']"]:
            ...
            if el:
                naslov = el.get_text(strip=True)
                break
        if not naslov:
            naslov = a.get("title", "") or a.get_text(strip=True)[:150]

        # Cena
        cena = None
        for sel in ["[class*='price']", "[class*='cena']", "[class*='Price']"]:
            el = parent.select_one(sel)
            if el:
        ...
        m2 = parse_m2(naslov)
        if not m2:
            for el in parent.select("span, li, div"):
    ...
    print(f"    Parser nasao {len(results)} oglasa sa stranice")
    return results

    # Stari kod ispod (nikad se ne izvrsava)
...
def scrape_mode(mode: str, session: requests.Session) -> list[dict]:
    ...
    for page in range(1, MAX_PAGES + 1):
        url  = base_p1 if page == 1 else base.format(page)
        print(f"    [{mode}] Stranica {page}: {url}")
        soup = fetch(url, session, page=page)
        ...
        if h1:
            import re as _re
            cnt_m = _re.search(r"(\d+)\s+rezultat", h1.get_text())
