...
DIRECT_HEADERS = {
    ...
    "Sec-Fetch-Site": "none",
    "Referer": "https://www.nekretnine.rs/",
}

def fetch(url: str, session: requests.Session, render: bool = False) -> BeautifulSoup | None:
    """Fetch URL direktno (bez proxy-ja) — nekretnine.rs ne blokira cloud IP-ove."""
    for attempt in range(1, 4):
        try:
            r = session.get(url, headers=DIRECT_HEADERS, timeout=30, allow_redirects=True)
            print(f"    Fetch {url} → {r.status_code} (final: {r.url})")
            if r.status_code == 200 and len(r.text) > 500:
                return BeautifulSoup(r.text, "html.parser")
            if r.status_code in (429, 503):
                print(f"    Rate limit — čekam {10 * attempt}s")
                time.sleep(10 * attempt)
            elif r.status_code in (403, 401):
                print(f"    Blokiran — pokušaj {attempt}/3")
