...
// ── AGENCIJE TAB ─────────────────────────────────────────────────────────────
function AgencijeTab({ mode, listings, mapping }) {
  ...
  return (
    <div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(180px,1fr))",gap:12,marginBottom:24}}>
        <KPI label={`Agencija aktivnih (${isProdaja?"prodaja":"renta"})`}
          value={agStats.length}
          sub="sa bar jednim oglasom"/>
        <KPI label="Oglasa preko agencija"
          value={fmt(total)}
          sub={`${Math.round(total/totalAll*100)}% od ukupnih ${totalAll}`}/>
        <KPI label="Lider tržišta"
          value={agStats[0]?.naziv||"–"}
          sub={agStats[0]?`${agStats[0].count} oglasa`:""}
          valueColor={accentCol}/>
        <KPI label="Top 3 agencije"
          value={`${Math.round((agStats.slice(0,3).reduce((s,a)=>s+a.count,0))/total*100)||0}%`}
          sub="udeo tržišta"/>
      </div>
        ...
        <div style={{
          ...
          <div>
            <div style={{fontSize:15,fontWeight:700,color:"#fff",marginBottom:2}}>
              Rang lista agencija — {isProdaja?"Prodaja":"Izdavanje"}
            </div>
            <div style={{fontSize:12,color:"rgba(255,255,255,.55)"}}>
              {agStats.length} aktivnih agencija · sortirano po broju oglasa
            </div>
          </div>
          <div style={{
            padding:"6px 14px",borderRadius:20,fontSize:12,fontWeight:600,
            background: isProdaja?"rgba(255,255,255,.15)":"rgba(59,130,246,.3)",
            color:"#fff",
          }}>
          ...
          <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
            <thead>
              <tr style={{background:"#F8FAFC",borderBottom:`2px solid ${C.border}`}}>
                <th style={{width:"52px",padding:"12px 16px 12px 24px",textAlign:"center",fontSize:11,fontWeight:700,color:C.textS,letterSpacing:.4,textTransform:"uppercase"}}>Rang</th>
                ...
                <th style={{width:"120px",padding:"12px 20px",textAlign:"right",fontSize:11,fontWeight:700,color:C.textS,letterSpacing:.4,textTransform:"uppercase"}}>Oglasi</th>
                <th style={{width:"200px",padding:"12px 24px 12px 8px",textAlign:"left",fontSize:11,fontWeight:700,color:C.textS,letterSpacing:.4,textTransform:"uppercase"}}>Tržišni udeo</th>
              </tr>
            </thead>
            <tbody>
              {agStats.map((a,i)=>{
                const pct      = Math.round(a.count/total*100*10)/10;
                const barPct   = Math.round(a.count/maxCount*100);
                const isTop3   = i < 3;
                const med      = medal(i);
                const rowBg    = i===0?"linear-gradient(90deg,#1B2A4A08,transparent)":
                                 i===1?"linear-gradient(90deg,#6B728008,transparent)":
                                 i===2?"linear-gradient(90deg,#F59E0B08,transparent)":
                                 "transparent";
                return (
                  <tr key={a.slug}
                    style={{
                    ...
                    <td style={{padding:"14px 20px"}}>
                      <div style={{display:"flex",alignItems:"center",gap:10}}>
                        <div style={{
                          ...
                          background:isTop3?accentCol:C.bg,
                          display:"flex",alignItems:"center",justifyContent:"center",
                          fontSize:13,fontWeight:700,
                          color:isTop3?"#fff":C.textS,
                        }}>
                        ...
                        <div>
                          <div style={{display:"flex",alignItems:"center",gap:6}}>
                            ...
                            <a href={`https://www.halooglasi.com/oglasi/${a.slug}`}
                              target="_blank" rel="noreferrer"
                              title="Svi oglasi na Halo Oglasima"
                              style={{
                                display:"inline-flex",alignItems:"center",justifyContent:"center",
                                width:20,height:20,borderRadius:6,flexShrink:0,
                                background:C.bg,color:C.textS,
                              ...
                              onMouseLeave={e=>{e.currentTarget.style.background=C.bg;e.currentTarget.style.color=C.textS;e.currentTarget.style.borderColor=C.border;}}
                            >↗</a>
                          </div>
                          <div style={{fontSize:11,color:C.textXS,marginTop:1}}>{a.slug}</div>
                        </div>
                      </div>
                    </td>
                    ...
                    <td style={{padding:"14px 20px",textAlign:"right"}}>
                      <span style={{
                        ...
                        background:isTop3?accentCol+"18":C.bg,
                        color:isTop3?accentCol:C.text,
                      }}>{a.count}</span>
                    </td>
                      ...
                      <div style={{display:"flex",alignItems:"center",gap:10}}>
                        <div style={{flex:1,height:6,background:"#F1F3F5",borderRadius:3,overflow:"hidden"}}>
                          <div style={{
                            ...
                            background:isTop3
                              ? `linear-gradient(90deg,${accentCol},${accentCol}99)`
                              : "#CBD5E1",
                            transition:"width .3s ease",
                          }}/>
                        </div>
                        <span style={{
                          minWidth:38,fontSize:12,fontWeight:600,
                          textAlign:"right",
                          color:isTop3?accentCol:C.textS,
                        }}>{pct}%</span>
                      </div>
                    </td>
                  </tr>
...
export default function Dashboard() {
  ...
  // Listinzi za agencije tab — samo aktuelni mode
  const agListings = useMemo(()=>
    mode==="prodaja" ? (latestP?.listings??[]) : (latestR?.listings??[])
  ,[mode, latestP, latestR]);
      ...
      <div style={{padding:"16px",maxWidth:1200,margin:"0 auto"}}>
        ...
        <div style={{display:"flex",gap:0,borderBottom:`1px solid ${C.border}`,marginBottom:20,overflowX:"auto"}}>
          {[["pregled","Segmentacija"],["trend","Trend"],["listinzi","Listinzi"],["agencije","Agencije"]].map(([k,l])=>(
            ...
            }}>{l}{k==="agencije"&&<span style={{marginLeft:6,fontSize:11,padding:"2px 7px",borderRadius:10,background:mode==="prodaja"?C.navy+"18":C.blue+"18",color:mode==="prodaja"?C.navy:C.blue,fontWeight:600}}>{mode==="prodaja"?"P":"R"}</span>}</button>
          ))}
        </div>
