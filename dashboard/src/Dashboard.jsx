...
const C = {
  navy:"#1B2A4A",bg:"#F4F5F7",white:"#FFFFFF",text:"#111827",
  textS:"#6B7280",textXS:"#9CA3AF",border:"#E5E7EB",
      ...
      <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
        ...
        {selected.map(z=>(
          <div key={z} style={{
            ...
            padding:"5px 10px 5px 14px",borderRadius:20,fontSize:12,fontWeight:600,
            background:C.navy+"15",color:C.navy,border:`1px solid ${C.navy}40`,
          }}>
            {z.replace("BW ","")}
...
function SortTH({ label, sortKey:k, activeSortKey, sortDir, onSort, width, align="left" }) {
  ...
  return (
    <th onClick={k?()=>onSort(k):undefined}
      ...
      style={{width,textAlign:align,padding:"10px 16px",fontSize:11,fontWeight:600,
        color:isActive?C.navy:hover?C.navy:C.textS,
        background:hover&&k?"#F3F4F6":"transparent",
        cursor:k?"pointer":"default",userSelect:"none",transition:"color .12s,background .12s"}}>
      {label}
      {k&&<span style={{marginLeft:4,fontSize:10,color:isActive?C.blue:C.textXS}}>
        {isActive?(sortDir===1?"↑":"↓"):"↕"}
      </span>}
    </th>
  );
...
// Tab Agencije — tabela sa prodaja + renta statistikom
function AgencijeTab({ listingsProdaja, listingsRenta, mapping }) {
  const [sortKey, setSortKey] = useState("prodaja");
  const [sortDir, setSortDir] = useState(-1);

  const agStats = useMemo(()=>{
    const map = {};
    const process = (listings, mode) => {
      for (const l of listings) {
        const slug = l.agencija;
        if (!slug) continue;
        const naziv = mapping[slug] || slug;
        if (!map[naziv]) map[naziv] = {naziv, slug, prodaja:0, renta:0, ukupno:0};
        map[naziv][mode]++;
        map[naziv].ukupno++;
      }
    };
    process(listingsProdaja, "prodaja");
    process(listingsRenta, "renta");
    return Object.values(map);
  },[listingsProdaja, listingsRenta, mapping]);

  const sorted = useMemo(()=>{
    return [...agStats].sort((a,b)=>(b[sortKey]-a[sortKey])*sortDir || a.naziv.localeCompare(b.naziv));
  },[agStats, sortKey, sortDir]);
  ...
  const totalProdaja = listingsProdaja.filter(l=>l.agencija).length;
  const totalRenta   = listingsRenta.filter(l=>l.agencija).length;
  const ukupnoAg     = agStats.length;
  const maxUkupno    = Math.max(...agStats.map(a=>a.ukupno), 1);

  return (
    <div>
      {/* KPI row */}
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(160px,1fr))",gap:12,marginBottom:20}}>
        <KPI label="Ukupno agencija" value={ukupnoAg} sub="sa aktivnim oglasima"/>
        <KPI label="Oglasa prodaja" value={fmt(totalProdaja)} sub="sa poznatom agencijom"/>
        <KPI label="Oglasa renta" value={fmt(totalRenta)} sub="sa poznatom agencijom"/>
      </div>

      {/* Tabela */}
      <div style={{background:C.white,borderRadius:12,boxShadow:C.shadow,overflow:"hidden"}}>
        <div style={{padding:"12px 16px",borderBottom:`1px solid ${C.border}`,fontSize:13,color:C.textS}}>
          {sorted.length} agencija — sortiraj po kolonama
        </div>
        <div style={{overflowX:"auto"}}>
          <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
            <thead style={{background:"#F9FAFB"}}>
              <tr style={{borderBottom:`1px solid ${C.border}`}}>
                <th style={{textAlign:"left",padding:"10px 16px",fontSize:11,fontWeight:600,color:C.textS,width:"5%"}}>#</th>
                <th style={{textAlign:"left",padding:"10px 16px",fontSize:11,fontWeight:600,color:C.textS,width:"35%"}}>Agencija</th>
                <SortTH label="Prodaja" sortKey="prodaja" activeSortKey={sortKey} sortDir={sortDir} onSort={toggleSort} width="15%" align="right"/>
                <SortTH label="Renta"   sortKey="renta"   activeSortKey={sortKey} sortDir={sortDir} onSort={toggleSort} width="15%" align="right"/>
                <SortTH label="Ukupno"  sortKey="ukupno"  activeSortKey={sortKey} sortDir={sortDir} onSort={toggleSort} width="30%" align="right"/>
              </tr>
            </thead>
            <tbody>
              {sorted.map((a,i)=>{
                const pct = Math.round(a.ukupno/maxUkupno*100);
                return (
                  <tr key={a.slug}
                    style={{borderBottom:`1px solid ${C.border}`}}
                    onMouseEnter={e=>e.currentTarget.style.background="#F9FAFB"}
                    onMouseLeave={e=>e.currentTarget.style.background="transparent"}>
                    <td style={{padding:"10px 16px",color:C.textXS,fontSize:12}}>{i+1}</td>
                    <td style={{padding:"10px 16px",fontWeight:500}}>{a.naziv}</td>
                    <td style={{padding:"10px 16px",textAlign:"right"}}>
                      {a.prodaja>0
                        ? <span style={{fontWeight:600,color:C.navy}}>{a.prodaja}</span>
                        : <span style={{color:C.textXS}}>–</span>}
                    </td>
                    <td style={{padding:"10px 16px",textAlign:"right"}}>
                      {a.renta>0
                        ? <span style={{fontWeight:600,color:C.blue}}>{a.renta}</span>
                        : <span style={{color:C.textXS}}>–</span>}
                    </td>
                    <td style={{padding:"10px 16px"}}>
                      <div style={{display:"flex",alignItems:"center",gap:8,justifyContent:"flex-end"}}>
                        <div style={{flex:1,maxWidth:120,height:6,background:"#F3F4F6",borderRadius:3,overflow:"hidden"}}>
                          <div style={{width:pct+"%",height:"100%",background:C.navy,borderRadius:3}}/>
                        </div>
                        <span style={{fontWeight:700,minWidth:24,textAlign:"right"}}>{a.ukupno}</span>
                      </div>
                    </td>
                  </tr>
                );
...
export default function Dashboard() {
  ...
  const [latestR, setLatestR] = useState(null);
  ...
  useEffect(() => {
    setLoading(true); setShowNew(false); setSelBlds([]);
    Promise.all([
      fetch(`${REPO}/latest_prodaja.json`).then(r=>r.json()),
      fetch(`${REPO}/latest_renta.json`).then(r=>r.json()),
      fetch(`${REPO}/history.json`).then(r=>r.json()).catch(()=>[]),
      fetch(`${REPO}/agencije_mapping.json`).then(r=>r.json()).catch(()=>({})),
    ])
    .then(([p,r,h,m])=>{
      setLatestP(p); setLatestR(r); setHist(h); setAgMapping(m);
      setLatest(mode==="prodaja"?p:r);
      setLoading(false);
    })
  ...
  useEffect(()=>{
    if(!latestP||!latestR) return;
    setLatest(mode==="prodaja"?latestP:latestR);
    setSelBlds([]); setShowNew(false);
  },[mode, latestP, latestR]);
  ...
  const segByStr = useMemo(()=>{
    ...
    return Object.fromEntries(Object.entries(result).map(([s,v])=>[s,{
      ...v, cena:agg(v.cene), cena_m2:agg(v.cene_m2), m2:agg(v.m2s),
    }]));
  },[bldFiltered]);
  ...
  const filtered = useMemo(()=>{
    ...
    return d.slice().sort((a,b)=>{
      const v=x=>sortKey==="zgrada"?(x.zgrada||""):sortKey==="str"?parseFloat(x.struktura||99):sortKey==="m2"?(x.m2||0):sortKey==="cena"?(x.cena||0):sortKey==="agencija"?(agMapping[x.agencija]||x.agencija||""):(x.cena_m2||0);
      const va=v(a),vb=v(b);
      if(typeof va==="string") return va.localeCompare(vb)*sortDir;
      return (va-vb)*sortDir;
    });
  },[listings,showNew,newKeys,selStr,selBlds,search,sortKey,sortDir,agMapping]);
    ...
    <div style={{fontFamily:"-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif",background:C.bg,minHeight:"100vh",fontSize:13,color:C.text}}>
      ...
      <div style={{background:C.navy,padding:"0 16px",display:"flex",alignItems:"center",height:52}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          ...
          <span style={{color:"rgba(255,255,255,.5)",fontSize:10}}>Update {scraped}</span>
        </div>
      </div>
      ...
      <div style={{padding:"16px",maxWidth:1200,margin:"0 auto"}}>
        ...
        {/* Info bar */}
        <div style={{background:C.white,borderRadius:10,padding:"10px 16px",marginBottom:16,display:"flex",alignItems:"center",gap:8,boxShadow:C.shadow,flexWrap:"wrap"}}>
        ...
        {/* AGENCIJE TAB — bez filtera zgrada */}
        {tab==="agencije" && (
          <AgencijeTab
            listingsProdaja={latestP?.listings??[]}
            listingsRenta={latestR?.listings??[]}
            mapping={agMapping}
          />
        )}

        {/* Ostali tabovi — sa filterima */}
        {tab!=="agencije" && (<>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(140px,1fr))",gap:12,marginBottom:20}}>
          <KPI label="Unique nekretnine"
            value={fmt(summary.cnt)}
            sub={isFiltered?`od ${latest?.total_unique} ukupno`:`${latest?.total_raw??0} oglasa, ${latest?.total_dups??0} dup.`}/>
          <KPI label="Duplikati"
            value={fmt(summary.dups)}
            sub={isFiltered?"za selektovane zgrade":"ista nkrt, više agencija"}/>
          <div onClick={()=>{setShowNew(true);setTab("listinzi");}} style={{cursor:"pointer"}}>
            <KPI label="Novi danas ↗"
              value={`+${diffSummary.newCount}`}
              sub={`−${diffSummary.removedCount} skinuto · klikni`}
              valueColor={C.green}/>
          </div>
          <KPI label="Cena raspon"
            value={summary.minC?(mode==="renta"?`${fmtKRenta(summary.minC)}–${fmtKRenta(summary.maxC)} €`:`${fmtK(summary.minC)}–${fmtK(summary.maxC)} €`):"–"}/>
          <KPI label="Prosek €/m²"
            value={summary.avgM2?`${fmt(summary.avgM2)} €`:"–"}
            sub={isFiltered?"selektovane zgrade":"sve strukture"}/>
              ...
              <div style={{background:C.white,borderRadius:12,padding:"20px",boxShadow:C.shadow,gridColumn:"1/-1"}}>
                ...
                <div style={{overflowX:"auto"}}>
                  <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
                    <thead>
                      <tr style={{borderBottom:`1px solid ${C.border}`}}>
                        {["Datum","Raw","Unique","Dup.","Novi","Skinuti","Avg €/m²"].map(h=>(
                          <th key={h} style={{textAlign:"left",padding:"8px 10px",fontSize:11,fontWeight:600,color:C.textS}}>{h}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
          ...
          <div style={{background:C.white,borderRadius:12,boxShadow:C.shadow,overflow:"hidden"}}>
            <div style={{padding:"12px 16px",borderBottom:`1px solid ${C.border}`,display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}>
              <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Pretraži oglase, agenciju..."
                style={{flex:1,minWidth:140,fontSize:13,padding:"7px 12px",border:`1px solid ${C.border}`,borderRadius:8,outline:"none",color:C.text}}/>
              {showNew&&(
                <span style={{padding:"4px 10px",borderRadius:20,background:C.green+"18",color:C.green,fontSize:12,fontWeight:600}}>
                  🟢 Novi ({diffSummary.newCount})
                </span>
              )}
              <span style={{fontSize:12,color:C.textS}}>{filtered.length} res.</span>
              {(selStr||selBlds.length>0||search||showNew)&&(
                <button onClick={()=>{setSelStr(null);setSelBlds([]);setSearch("");setShowNew(false);}} style={{
                  fontSize:12,padding:"6px 10px",cursor:"pointer",border:`1px solid ${C.border}`,
              ...
              <table style={{width:"100%",borderCollapse:"collapse",fontSize:13,tableLayout:"fixed"}}>
                ...
                <tbody>
                  {filtered.map(l=>{
                    ...
                    const agNaziv = l.agencija ? (agMapping[l.agencija]||l.agencija) : null;
                    return (
                      <tr key={l.id}
                        ...
                        <td style={{padding:"10px 16px",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",fontSize:12,color:agNaziv?C.text:C.textXS}}>
                          {agNaziv||"–"}
                        </td>
                        <td style={{padding:"10px 16px"}}>
