...
function AgencijeTab({ mode, listings, agMapping, agMode }) {
  const agStats = useMemo(()=>{
    ...
    for (const l of listings) {
      ...
      if (!raw) continue;
      // Halo: slug → mapping lookup; NRS: puno ime direktno
          ...
          <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
            ...
            <tbody>
              {agStats.map((a,i)=>{
                ...
                // Link URL: Halo → /oglasi/slug, NRS → direktno na sajt
                const haloLink = agMode === "slug" && a.slug ? `https://www.halooglasi.com/oglasi/${a.slug}` : null;
                return (
                      ...
                      <div style={{display:"flex",alignItems:"center",gap:10}}>
                        ...
                        <div>
                          <div style={{display:"flex",alignItems:"center",gap:6}}>
                            ...
                            {haloLink && (
                              <a href={haloLink} target="_blank" rel="noreferrer" title="Svi oglasi na Halo Oglasima"
                          ...
                          {a.slug && <div style={{fontSize:11,color:C.textXS,marginTop:1}}>{a.slug}</div>}
                        </div>
                      </div>
                    </td>
...
export default function Dashboard() {
  ...
  const histSlice = useMemo(()=>{
    // history_nrs ima mode field, halo nema
