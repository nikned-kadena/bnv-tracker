...
// ── Collapsible Building Filter ───────────────────────────────────────────────
function BuildingFilter({ buildings, selected, onSelect }) {
  const [open, setOpen] = useState(false);
  const selectedLabel = selected ? selected.replace("BW ", "") : null;
    ...
    <div style={{ marginBottom: 10 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
        ...
        >
          ...
          <span style={{
            ...
            background: "rgba(255,255,255,.2)", fontSize: 10, fontWeight: 700,
        ...
        {selected && (
          <div style={{
            display: "flex", alignItems: "center", gap: 6,
            padding: "5px 10px 5px 14px", borderRadius: 20, fontSize: 13, fontWeight: 600,
      ...
      {open && (
        <div style={{
          ...
          <Pill label="Sve zgrade" active={!selected} onClick={() => { onSelect(null); setOpen(false); }} />
          {buildings.map(z => (
            <Pill
              key={z}
              label={z.replace("BW ", "")}
              active={selected === z}
              onClick={() => { onSelect(selected === z ? null : z); setOpen(false); }}
            />
          ))}
        </div>
      )}
...
function SortTH({ label, sortKey: k, activeSortKey, sortDir, onSort, width }) {
  ...
  return (
    <th
      ...
      onMouseEnter={() => k && setHover(true)}
      onMouseLeave={() => setHover(false)}
      style={{
        width, textAlign: "left", padding: "10px 16px", fontSize: 11, fontWeight: 600,
    ...
    >
      {label}
      {k && (
        <span style={{ marginLeft: 4, fontSize: 10, color: isActive ? C.blue : C.textXS }}>
          {isActive ? (sortDir === 1 ? "↑" : "↓") : "↕"}
        </span>
      )}
    </th>
    ...
    <div style={{ fontFamily:"-apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif", background:C.bg, minHeight:"100vh", fontSize:13, color:C.text }}>
      ...
      <div style={{ background:C.navy, padding:"0 16px", display:"flex", alignItems:"center", height:52 }}>
        <div style={{ display:"flex", alignItems:"center", gap:10, marginRight:"auto" }}>
          ...
          <span style={{ color:"rgba(255,255,255,.4)", fontSize:12, marginLeft:4, display:"none" }} className="desktop-only">Beograd na vodi · Savski venac</span>
        </div>
      ...
      <div style={{ padding:"16px", maxWidth:1200, margin:"0 auto" }}>
        ...
        <div style={{ background:C.white, borderRadius:10, padding:"10px 16px", marginBottom:16, display:"flex", alignItems:"center", gap:8, boxShadow:C.shadow, flexWrap:"wrap" }}>
          <span style={{ fontSize:12, color:C.textS }}>
            📅 <strong style={{ color:C.text }}>{scraped}</strong>
          </span>
          <span style={{ marginLeft:"auto", fontSize:12, color:C.textS }}>{now}</span>
        </div>

        {/* ── Zgrada filter — collapsible ── */}
        <BuildingFilter
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(140px,1fr))", gap:12, marginBottom:20 }}>
          <KPI label="Unique nekretnine"   value={fmt(latest?.total_unique)}  sub={`${latest?.total_raw ?? 0} oglasa, ${latest?.total_dups ?? 0} dup.`} />
