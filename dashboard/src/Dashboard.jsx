    ...
    <div style={{marginBottom:10}}>
      ...
      {open&&(
        <div style={{marginTop:10,padding:"12px 14px",background:C.white,borderRadius:12,boxShadow:C.shadowM,display:"flex",gap:6,flexWrap:"wrap"}}>
          ...
          {buildings.map(z=>(
            <Pill key={z} label={z.replace("BW ","")} active={selected.has(z)} onClick={()=>onToggle(z)}/>
          ))}
        </div>
      )}
    </div>
...
export default function Dashboard() {
  ...
  // Listings filtrirani po zgradama (za segmentaciju i KPIje)
  const bldFiltered = useMemo(()=>
    selBlds.size>0 ? listings.filter(l=>selBlds.has(l.zgrada)) : listings
  ,[listings,selBldKey]);

  // Segmentacija po strukturi — računata iz filtriranih listinga
  ...
  },[bldFiltered,selBldKey,latest]);

  const diffSummary = useMemo(()=>({
    newCount:     selBlds.size>0?(diff.new??[]).filter(l=>selBlds.has(l.zgrada)).length:(diff.new?.length??0),
    removedCount: selBlds.size>0?(diff.removed??[]).filter(l=>selBlds.has(l.zgrada)).length:(diff.removed?.length??0),
  }),[diff,selBldKey]);
  ...
  },[listings,showNew,newKeys,selStr,selBldKey,search,sortKey,sortDir]);
  ...
  // Stabilan string key za Set — triggera useMemo kad se Set promeni
  const selBldKey = useMemo(()=>[...selBlds].sort().join("|"),[selBlds]);
  ...
  const isFiltered = selBlds.size > 0;
  ...
  return (
    <div style={{fontFamily:"-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif",background:C.bg,minHeight:"100vh",fontSize:13,color:C.text}}>
      ...
      <div style={{padding:"16px",maxWidth:1200,margin:"0 auto"}}>
        ...
        {/* PREGLED — koristi segByStr koji je filtriran po zgradama */}
        {tab==="pregled"&&(
              ...
              <table style={{width:"100%",borderCollapse:"collapse",fontSize:13,tableLayout:"fixed"}}>
                <thead style={{background:"#F9FAFB"}}>
                  <tr style={{borderBottom:`1px solid ${C.border}`}}>
                    ...
                    <SortTH label={mode==="prodaja"?"Cena":"Renta"} sortKey="cena" activeSortKey={sortKey} sortDir={sortDir} onSort={toggleSort} width="16%" align="right"/>
