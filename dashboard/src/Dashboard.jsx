...
// Collapsible multi-select filter za zgrade
function BuildingFilter({ buildings, selected, onToggle, onClear }) {
  ...
  return (
    <div style={{marginBottom:10}}>
      <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
        ...
        {[...selected].map(z=>(
          <div key={z} style={{
            ...
            {z.replace("BW ","")}
            <button onClick={()=>onToggle(z)} style={{
              background:"none",border:"none",cursor:"pointer",
              color:C.navy,fontSize:14,lineHeight:1,padding:"0 2px",
            }}>×</button>
          </div>
        ))}

        {count===0&&!open&&(
          <span style={{fontSize:12,color:C.textS}}>{buildings.length} zgrada</span>
        )}
        {count>0&&(
          <button onClick={onClear} style={{
            fontSize:12,color:C.textS,background:"none",border:"none",cursor:"pointer",padding:"4px 8px",
          }}>Obriši sve ×</button>
        )}
      </div>
      ...
      {open&&(
        <div style={{
          marginTop:10,padding:"12px 14px",background:C.white,borderRadius:12,
          boxShadow:C.shadowM,display:"flex",gap:6,flexWrap:"wrap",
        }}>
          {buildings.map(z=>(
            <Pill key={z} label={z.replace("BW ","")} active={selected.has(z)}
              onClick={()=>{ onToggle(z); }} />
          ))}
        </div>
      )}
...
function SortTH({ label, sortKey:k, activeSortKey, sortDir, onSort, width }) {
  ...
  return (
    <th onClick={k?()=>onSort(k):undefined}
      ...
      style={{width,textAlign:"left",padding:"10px 16px",fontSize:11,fontWeight:600,
...
export default function Dashboard() {
  ...
  const [selBlds, setSelBlds] = useState(new Set()); // višestruki odabir
  ...
  useEffect(() => {
    ...
    setLoading(true); setShowNew(false); setSelBlds(new Set());
    Promise.all([
  ...
  },[listings,showNew,newKeys,selStr,selBlds,search,sortKey,sortDir]);

  // KPI summary — reaguje na filter zgrada
  const summary = useMemo(()=>{
    const src = selBlds.size>0 ? listings.filter(l=>selBlds.has(l.zgrada)) : listings;
    const prices=src.filter(l=>l.cena).map(l=>l.cena);
    const m2s=src.filter(l=>l.cena_m2).map(l=>l.cena_m2);
  ...
  // Diff KPIji — reaguju na filter zgrada
  const diffSummary = useMemo(()=>{
    if(selBlds.size===0) return {newCount:diff.new?.length??0, removedCount:diff.removed?.length??0};
    const newCount=(diff.new??[]).filter(l=>selBlds.has(l.zgrada)).length;
    const removedCount=(diff.removed??[]).filter(l=>selBlds.has(l.zgrada)).length;
    return {newCount,removedCount};
  },[diff,selBlds]);

  const toggleSort=k=>{if(sortKey===k)setSortDir(d=>-d);else{setSortKey(k);setSortDir(1);}};
  const toggleBld=z=>setSelBlds(prev=>{const n=new Set(prev);n.has(z)?n.delete(z):n.add(z);return n;});
  ...
  return (
    <div style={{fontFamily:"-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif",background:C.bg,minHeight:"100vh",fontSize:13,color:C.text}}>
      ...
      <div style={{padding:"16px",maxWidth:1200,margin:"0 auto"}}>
        ...
        {/* Zgrada filter */}
        <BuildingFilter buildings={allBuildings} selected={selBlds} onToggle={toggleBld} onClear={()=>setSelBlds(new Set())} />
        ...
        {/* KPI row — reaguje na filter zgrada */}
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(140px,1fr))",gap:12,marginBottom:20}}>
          <KPI label="Unique nekretnine"
            value={fmt(selBlds.size>0 ? summary.cnt : latest?.total_unique)}
            sub={selBlds.size>0 ? `od ${latest?.total_unique} ukupno` : `${latest?.total_raw??0} oglasa, ${latest?.total_dups??0} dup.`}/>
          <KPI label="Duplikati" value={fmt(latest?.total_dups)} sub="ista nkrt, više agencija"/>
          ...
          <KPI label="Prosek €/m²" value={summary.avgM2?`${fmt(summary.avgM2)} €`:"–"} sub={selBlds.size>0?"selektovane zgrade":"sve strukture"}/>
          <KPI label="DoD" value={fmtPct(priceIdx.dod)} sub="vs juče" valueColor={pctColor(priceIdx.dod)}/>
          <KPI label="YTD" value={fmtPct(priceIdx.ytd)} sub="od 01.01." valueColor={pctColor(priceIdx.ytd)}/>
        </div>

        {/* Tab nav */}
        <div style={{display:"flex",gap:0,borderBottom:`1px solid ${C.border}`,marginBottom:20,overflowX:"auto"}}>
          {[["pregled","Segmentacija"],["trend","Trend"],["listinzi","Listinzi"]].map(([k,l])=>(
        ...
        {tab==="pregled"&&(
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(280px,1fr))",gap:16}}>
            {STR_ORDER.filter(s=>byStr[s]&&(!selStr||selStr===s)).map(s=>{
              const v=byStr[s],col=STR_COLOR[s];
        ...
        {tab==="listinzi"&&(
          <div style={{background:C.white,borderRadius:12,boxShadow:C.shadow,overflow:"hidden"}}>
            <div style={{padding:"12px 16px",borderBottom:`1px solid ${C.border}`,display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}>
              ...
              {(selStr||selBlds.size>0||search||showNew)&&(
                <button onClick={()=>{setSelStr(null);setSelBlds(new Set());setSearch("");setShowNew(false);}} style={{
            ...
            <div style={{overflowX:"auto"}}>
              <table style={{width:"100%",borderCollapse:"collapse",fontSize:13,tableLayout:"fixed"}}>
                <thead style={{background:"#F9FAFB"}}>
                  <tr style={{borderBottom:`1px solid ${C.border}`}}>
                    <SortTH label="Zgrada" sortKey="zgrada" activeSortKey={sortKey} sortDir={sortDir} onSort={toggleSort} width="30%"/>
                    <SortTH label="Tip"    sortKey="str"    activeSortKey={sortKey} sortDir={sortDir} onSort={toggleSort} width="15%"/>
                    <SortTH label="m²"     sortKey="m2"     activeSortKey={sortKey} sortDir={sortDir} onSort={toggleSort} width="9%"/>
                    <SortTH label={mode==="prodaja"?"Cena":"Renta"} sortKey="cena" activeSortKey={sortKey} sortDir={sortDir} onSort={toggleSort} width="17%"/>
                    {mode==="prodaja"&&<SortTH label="€/m²" sortKey="m2p" activeSortKey={sortKey} sortDir={sortDir} onSort={toggleSort} width="12%"/>}
                    <SortTH label="Sprat" sortKey={null} activeSortKey={sortKey} sortDir={sortDir} onSort={toggleSort} width="10%"/>
                    <SortTH label=""      sortKey={null} activeSortKey={sortKey} sortDir={sortDir} onSort={toggleSort} width="7%"/>
                  </tr>
                </thead>
                <tbody>
                  {filtered.map(l=>{
                    ...
                    return (
                      <tr key={l.id}
                        ...
                        <td style={{padding:"10px 16px",color:C.textS}}>{l.m2??"–"}</td>
